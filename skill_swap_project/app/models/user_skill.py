...
class UserSkill(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint('user_id', 'skill_name', 'skill_type', name='unique_user_skill_type'),
    )
    
    def __init__(self, user_id, skill_name, skill_type, description=None, proficiency_level='intermediate'):
    ...
    @classmethod
    def find_users_by_skill(cls, skill_name, skill_type='offered'):
        """Find users who offer or want a specific skill"""
        return cls.query.filter_by(skill_name=skill_name, skill_type=skill_type).all()
